...
(function(Global, alreadyGo) {
    ...
    //Object Method
        hasOwnProperty = Go_Object.hasOwnProperty,
    //Array Method
        push = Go_Array.push,
    ...
    //ECMAScript 5
        ...
        nativeMap = Go_Array.map,
        nativeForEach = Go_Array.forEach,
        nativeFilter = Go_Array.filter,
        nativeReduce = Go_Array.reduce,
        nativeReduceRight = Go_Array.reduceRight,
    ...
    Go.Extend = _Go.prototype = {
        ...
        insetHTML: function(obj) {
			...
			var fuckIE = this.selector[0];
			//if more than two nodes
			if((Go.isIE7 || Go.isIE6 ) && fuckIE[0] && fuckIE[1])
			   	Go.ObjEach(fuckIE,function(elm){ elm.innerHTML = getInset; });
			else
        ...
        gtPos: function() {
            return Go.getPosition(Go.GetRealNode(this.selector[0]));
        },
        scTo: function(scrollTime) {
            ...
            var z = {
                el: _Go.selector[0],
                p: Go.getPosition(Go.GetRealNode(_Go.selector[0])),
                s: Go.getScroll(),
                t: (new Date()).getTime(),
                scroll: function(t, l) {
                    global.scrollTo(l, t)
    ...
    // Go.AryReduce 依据一个基数和函数 依照从左到右的顺序进行迭代
    ...
    Go.AryReduce = function(ary, func, refer) {
    ...
    Go.AryReduceRight = function(ary, func, refer) {
        if (Go.isArray(ary))
